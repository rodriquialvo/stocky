              ...
              {productsSelected.map((product, index) => (
                <Box key={index}>
                  ...
                  >
                    ...
                    <Text fontWeight={"bold"}>{formattedNumberToMoney( roundUpTo100((product.prices.reseller + (product.prices.reseller * parseFloat(percentage))  / 100)))}</Text>
                    <Text>{formattedNumberToMoney(product.prices.retail)}</Text>
                    <Text fontWeight={"bold"}>{formattedNumberToMoney( roundUpTo100((product.prices.retail + (product.prices.retail * parseFloat(percentage)) / 100)))}</Text>
                    <IconButton
                      aria-label='Delete'
                      icon={<DeleteIcon />}
                      variant='ghost'
